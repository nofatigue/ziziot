...
function sendOrderConfirmationEmail(orderId, parentName, studentName, grade, email, totalPayment, orderDetails) {
  const subjectForCustomer = "הזמנת ציציות ישלצ";
  const subjectForOwner = `הזמנת ציציות ${studentName}`;
  let emailBody = ""
  emailBody += "orderId:" + `${orderId}` + "<br>"
  emailBody += `<p>לכבוד ${parentName},</p>`;
  emailBody += "כיתה:" + `${grade}` + "<br>"
  emailBody += `להלן פרטי הזמנת חומרי הכנת ציציות עבור התלמיד ${studentName}:<br>`;
